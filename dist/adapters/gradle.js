...
import { headerPath } from "../toml.js";
...
export function catalogSites(toml) {
    const sites = [];
    let section = '';
    let offset = 0;
    for (const line of toml.split('\n')) {
        ...
        const at = (value, index) => ({ value, start: rhsAt + index, end: rhsAt + index + value.length });
        if (section === 'versions') {
            // alias = "1.2.3", or a rich version { strictly/require/prefer = "…" }
            const q = /^\s*"([^"]+)"/.exec(rhs) ?? /(?:strictly|require|prefer)\s*=\s*"([^"]+)"/.exec(rhs);
            if (q)
                sites.push({ name: alias, section: 'catalog', ...at(q[1], q.index + q[0].indexOf(q[1])) });
        }
...
function versionRefs(toml) {
    ...
    for (const m of toml.matchAll(/^\s*[A-Za-z0-9_.-]+\s*=\s*\{([^}]*)\}/gm)) {
        ...
        if (module)
            refs.set(ref, [...(refs.get(ref) ?? []), module]);
    }
    return refs;
}
export function scriptSites(script) {
...
export const gradle = siteAdapter({
    ...
    members: (_snap, paths) => paths.filter((p) => (BUILD.test(p) || CATALOG.test(p)) && !/(^|\/)(build|\.gradle|node_modules)\//.test(p)),
