    ...
    string() {
        ...
        for (;;) {
            ...
            if (multi ? this.s.startsWith(q.repeat(3), this.i) : this.peek() === q) {
                ...
                if (multi)
                    while (this.s.startsWith(q.repeat(4), this.i))
                        (out += q), this.i++;
                this.i += multi ? 3 : 1;
                return out;
            }
    ...
    value() {
        ...
        if (c === '{') {
            ...
            for (;;) {
                const k = this.key();
                if (this.peek() !== '=')
                    this.error('expected = in inline table');
                this.i++;
        ...
        if (/^[+-]?(\d+(\.\d+)?([eE][+-]?\d+)?|0x[0-9a-fA-F]+|0o[0-7]+|0b[01]+|inf|nan)$/.test(num))
            return Number(num);
        return raw;
    }
}
function setPath(table, path, value) {
    ...
    for (const k of path.slice(0, -1)) {
        if (typeof t[k] !== 'object' || Array.isArray(t[k]))
            t[k] = {};
        t = t[k];
    }
        ...
        if (p.peek() === '[') {
            ...
            path.forEach((k, idx) => {
                ...
                if (Array.isArray(next))
                    next = next[next.length - 1]; // [[a]] then [a.b]
...
export function headerPath(line) {
...
const samePath = (a, b) => !!a && a.length === b.length && a.every((x, i) => x === b[i]);
/** Span of the whole `[table]` section (header through the line before the next header). */
export function findTable(text, table) {
    const starts = lineStarts(text);
    for (let li = 0; li < starts.length; li++) {
        const line = text.slice(starts[li], (starts[li + 1] ?? text.length + 1) - 1);
        if (!samePath(headerPath(line), table))
            continue;
        let lj = li + 1;
        while (lj < starts.length && headerPath(text.slice(starts[lj], (starts[lj + 1] ?? text.length + 1) - 1)) === null)
            lj++;
...
export function findEntry(text, table, key) {
...
export const splice = (text, span, replacement) => text.slice(0, span.start) + replacement + text.slice(span.end);
