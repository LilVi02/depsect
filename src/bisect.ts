...
export async function findCulprits<T>(
  ...
  const test = async (subset: T[]): Promise<Outcome> => {
  ...
  const minimize = async (candidates: T[], fixed: T[]): Promise<T[]> => {
    ...
    while (hi - lo > 1) {
      const mid = (lo + hi) >> 1;
      if ((await test([...fixed, ...candidates.slice(0, mid)])) === 'fail') hi = mid;
      else lo = mid;
    }
    const required = candidates[hi - 1]!;
    const nextFixed = [...fixed, required];
    if ((await test(nextFixed)) === 'fail') return nextFixed;
    return minimize(candidates.slice(0, hi - 1), nextFixed);
  };

  if ((await test([])) === 'fail') throw new BaseBrokenError();
  if ((await test(units)) === 'pass') throw new NoFailureError();
  ...
  for (;;) {
    ...
    if (remaining.length === 0 || (await test(remaining)) === 'pass') break;
  }
