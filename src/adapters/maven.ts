...
const clean = (xml: string) => mask(mask(xml, /<!--[\s\S]*?-->/g), /<!\[CDATA\[[\s\S]*?\]\]>/g);

/** First direct `<tag>text</tag>` in a block whose nested structures have been masked. */
function child(block: string, offset: number, tag: string): { text: string; start: number; end: number } | null {
...
const KINDS = ['dependency', 'plugin', 'parent', 'extension'] as const;

/** Coordinates ("group:artifact") that reference each ${property} as their version. */
function propertyUsers(xml: string): Map<string, string[]> {
...
export function pomSites(xml: string): Site[] {
  const text = clean(xml);
  ...
  for (const kind of KINDS) {
    for (const m of text.matchAll(new RegExp(`<${kind}>([\\s\\S]*?)</${kind}>`, 'g'))) {
      ...
      if (!artifact || !version || version.text.includes('${')) continue;
...
export const maven = siteAdapter({
  ...
  members: (_snap, paths) =>
    paths.filter((p) => posix.basename(p) === POM && p !== POM && !/(^|\/)(target|node_modules)\//.test(p)),
