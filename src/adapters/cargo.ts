...
import { findEntry, findTable, parseToml, splice, type TomlTable, type TomlValue } from '../toml.ts';
import { showVersions, type Adapter, type Snapshot, type Update } from './types.ts';
...
export function parseCargoLock(text: string | null | undefined): Crate[] {
  if (!text) return [];
...
const directNames = (crates: Crate[]) => new Set(crates.filter((c) => !c.source).flatMap((c) => c.deps));

/** Pair old and new versions of a crate: semver-compatible ones first, then leftovers if one of each. */
export function pairVersions(from: string[], to: string[]): [string, string][] {
  const compat = (v: string) => {
  ...
  if (f.length === 1 && t.length === 1) pairs.push([f[0]!, t[0]!]);
  return pairs;
}

/** Tables that can declare dependencies in Cargo.toml. */
function dependencyTables(man: TomlTable): string[][] {
  const kinds = ['dependencies', 'dev-dependencies', 'build-dependencies'];
  const paths: string[][] = kinds.map((k) => [k]);
  paths.push(['workspace', 'dependencies']);
  for (const target of Object.keys(table(man.target))) for (const k of kinds) paths.push(['target', target, k]);
  return paths;
}
...
function findDecl(text: string, crate: string): Decl | null {
  const man = parseToml(text);
  for (const path of dependencyTables(man)) {
    let t: TomlValue | undefined = man;
    for (const k of path) t = table(t)[k];
    for (const [key, value] of Object.entries(table(t))) {
      if (key !== crate && table(value).package !== crate) continue;
      const span = findEntry(text, path, key) ?? findTable(text, [...path, key]);
      if (span) return { table: path, ...span, text: text.slice(span.start, span.end) };
    }
  }
  return null;
}
...
export function editManifest(baseText: string, headText: string, crate: string): string {
  const b = findDecl(baseText, crate);
  const h = findDecl(headText, crate);
  if (b && h && b.text === h.text) return baseText;
  if (b && h && b.table.join('.') === h.table.join('.')) return splice(baseText, b, h.text);
  const without = b ? splice(baseText, b, '') : baseText;
  return h ? insertDecl(without, h) : without;
}

export const cargo: Adapter = {
  name: 'cargo',
  ...
  members(snap, paths) {
    ...
    const list = (v: TomlValue | undefined) => (Array.isArray(v) ? v.map(String) : []);
  ...
  diff(base, head) {
    ...
    const versions = (crates: Crate[]) => {
      const m = new Map<string, string[]>();
      for (const c of crates) if (c.source) (m.get(c.name) ?? m.set(c.name, []).get(c.name)!).push(c.version);
      return m;
    };
    const bv = versions(b);
    const hv = versions(h);
    ...
    for (const name of [...new Set([...bv.keys(), ...hv.keys()])].sort()) {
      ...
      if (kind === 'direct' && !bv.has(name)) updates.push({ id: name, name, section, from: null, to: showVersions(to), kind });
      else if (kind === 'direct' && !hv.has(name)) updates.push({ id: name, name, section, from: showVersions(from), to: null, kind });
  ...
  async write(dir, base, head, subset) {
    ...
    const manifests = [...new Set([...Object.keys(base), ...Object.keys(head)])].filter(
      (k) => (k === MANIFEST || k.endsWith(`/${MANIFEST}`)) && head[k] != null,
    );
    for (const m of manifests) {
      let text = base[m] ?? head[m]!;
      for (const u of subset.filter((x) => x.kind === 'direct')) text = editManifest(text, head[m]!, u.name);
    ...
    if (added.length || subset.some((x) => x.from && !x.to)) cmds.push('cargo update --workspace');
    for (const u of added) cmds.push(`cargo update -p ${q(u.name)} --precise ${q(u.to!)}`);
    return cmds;
  },
};
