...
import { safePrBody, safePrTitle, openSafePr } from './safe-pr.ts';
import { COMMENT_MARKER, toMarkdown } from './report.ts';
import { run, type TransitiveMode } from './runner.ts';

const input = (name: string): string => (process.env[`INPUT_${name.toUpperCase()}`] ?? '').trim();
const bool = (name: string) => input(name).toLowerCase() === 'true';
...
async function upsertComment(token: string, repo: string, pr: number, body: string) {
  const comments = (await gh(token, 'GET', `/repos/${repo}/issues/${pr}/comments?per_page=100`)) as {
    id: number;
    body?: string;
  }[];
  const existing = comments.find((c) => c.body?.includes(COMMENT_MARKER));
  if (existing) await gh(token, 'PATCH', `/repos/${repo}/issues/comments/${existing.id}`, { body });
  else await gh(token, 'POST', `/repos/${repo}/issues/${pr}/comments`, { body });
}

async function main() {
  const event: PullRequestEvent = process.env.GITHUB_EVENT_PATH
  ...
  if (!test) throw new Error('Input "test-command" is required');
  const base = input('base') || pr?.base.sha;
  if (!base) throw new Error('Input "base" is required outside pull_request events');
  const timeout = Number(input('timeout-minutes'));
  const transitive = (input('transitive') || 'auto') as TransitiveMode;
  if (!['auto', 'always', 'never'].includes(transitive)) throw new Error('Input "transitive" must be auto, always or never');
  ...
  if (bool('comment') && token && pr && repo && report.status !== 'no-updates') {
    try {
      ...
      console.log(`::warning::Could not comment on the PR (${(err as Error).message}). The report is in the job summary.`);
    }
  }
